...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='We be a music daddy'.format(BOTVERSION))
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run `{}help [command]`\n'
                                 'For further help, see the music daddy').format(prefix)
        if not self.is_all:
    ...
    async def cmd_play(self, message, player, channel, author, permissions, leftover_args, song_url):
          ...
          if song_url.startswith('krysten'):
              song_url = "spotify:user:12151865827:playlist:4pMdu1otSVSvWJCOOal0MK"
          if song_url.startswith('summer2018'):
              song_url = "spotify:user:krystenholley:playlist:44UMQGJfjuKw3cQqiSocB2"
          if song_url.startswith('summervibes2018'):
              song_url = "spotify:user:krystenholley:playlist:44UMQGJfjuKw3cQqiSocB2"
          if song_url.startswith('country'):
              song_url = "spotify:user:krystenholley:playlist:6xGSGcq1rvuC581c9MrdqR"
          if song_url.startswith('meme'):
              song_url = "spotify:user:krystenholley:playlist:1rs9MDDVRaoHMKDJjpIkOa"
          if song_url.startswith('algoma2018'):
              song_url = "spotify:user:krystenholley:playlist:05taugle5evwOxQNukkFAp"
          if song_url.startswith('justin'):
              song_url = "spotify:user:krystenholley:playlist:5aeeL5QoaZFmKy77rSJB23"
          if song_url.startswith('latenightchill'):
              song_url = "spotify:user:krystenholley:playlist:5HifvPab8kA6bEG2ux10GU"
          if song_url.startswith('winter2018'):
              song_url = "spotify:user:krystenholley:playlist:4p8uyBKjOBmb04pJEAGtH3"
          if song_url.startswith('fall2018'):
              song_url = "spotify:user:krystenholley:playlist:3eaK2Tj5uYGJcCfrCJZY8V"
          if song_url.startswith('wintermix2018'):
              song_url = "spotify:user:krystenholley:playlist:4p8uyBKjOBmb04pJEAGtH3"
          if song_url.startswith('falljams2018'):
              song_url = "spotify:user:krystenholley:playlist:3eaK2Tj5uYGJcCfrCJZY8V"
          if song_url.startswith('singwithme'):
              song_url = "spotify:user:krystenholley:playlist:5528v2nSuzBq0HOZbqBCTD"
